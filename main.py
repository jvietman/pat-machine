...
def merge(img1, img2, img3, id=0, pos1=vector2(0, 0), pos2=vector2(0, 0)):
    ...
    if config["squish"]:
        mg = img2.resize((config["size"][0], config["size"][1] + (config["squishsteps"][id] * config["squishyness"]))).convert("RGBA")
    else:
    ...
    if config["squish"]:
        posone = (pos2.x + config["center"][0] + tmppos[0], pos2.y + config["center"][1] + tmppos[1] - (config["squishsteps"][id] * config["squishyness"]))
    else:
        posone = (pos2.x + config["center"][0] + tmppos[0], pos2.y + config["center"][1] + tmppos[1])
...
# generate gif
frameone = frames[0]
frameone.save("output/"+config["filename"]+".gif", format="GIF", append_images=frames, save_all=True, loop=0, duration=config["speed"])
